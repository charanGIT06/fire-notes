      ...
      >
        <div className='card-heading px-1 px-md-3 d-flex flex-row justify-content-between align-items-center'>
          <Input
            ...
            defaultValue={presentNote.title}
            onChange={(e) =>
              setPresentNote({
                ...presentNote,
        ...
        <div className='card-content px-1 px-md-3 pe-2'>
          <Textarea
            variant='unstyled'
            isReadOnly={page === "shared" ? true : false}
            className='note-textarea p-0 m-0 mb-3'
            placeholder='Text goes here...'
            defaultValue={presentNote.content}
            onChange={(e) =>
              setPresentNote({
                ...presentNote,
        ...
        <div className='card-footer px-0 px-md-2 d-flex align-items-center'>
          <div
            className={`${
              page !== "shared" ? "d-none" : ""
          ...
          >
            <div className='d-flex ms-2'>
              <Tooltip label={`Shared by: ${presentNote?.owner?.email}`}>
                <div
                  className='owner d-flex flex-row align-items-center p-2'
                  style={{ border: "2px solid gray", borderRadius: "30px" }}
                >
                  <Avatar
                    name={presentNote?.owner?.displayName}
                    size={"sm"}
                    className='me-1'
                  />
                  <p className='p-0 m-0 ms-1'>
                    {presentNote?.owner?.displayName}
                  </p>
                </div>
              </Tooltip>
            </div>
          ...
          <div className='btns d-flex flex-row align-items-center justify-content-start w-100 mt-2'>
            <div className='left w-100 d-flex flex-row'>
              <Tooltip
                label={presentNote.isPinned === true ? "Unpin" : "Pin"}
                placement='top'
                hasArrow='true'
              >
                <IconButton
                  ...
                  size={"lg"}
                  icon={
                    presentNote.isPinned !== true ? (
                      <BsPinFill size={"22px"} />
                    ) : (
                      <BsPinAngleFill size={"22px"} />
                    )
                  }
                  onClick={() => {
                    setPresentNote({
              ...
              <Tooltip label='Archive' placement='top' hasArrow='true'>
                <IconButton
                  ...
                  variant='ghost'
                  size={"lg"}
                  icon={<BsFillArchiveFill size={"22px"} />}
                  onClick={() => {
                    navigate(`/${page === "notes" ? "" : page}`);
                    archiveNote();
                  }}
              ...
              <Tooltip label='Unarchive' placement='top' hasArrow='true'>
                <IconButton
                  ...
                  isRound={true}
                  color={`${theme === "dark" ? "#ADB5BD" : "#495057"}`}
                  className={`${page === "archive" ? "me-1" : "d-none"}`}
                  icon={<BiSolidArchiveOut size={"22px"} />}
                  onClick={() => {
                    navigate(`/${page === "notes" ? "" : page}`);
                    unArchiveNote();
                  }}
              ...
              <Tooltip label='Restore' placement='top' hasArrow='true'>
                <IconButton
                  ...
                  isRound={true}
                  color={`${theme === "dark" ? "#ADB5BD" : "#495057"}`}
                  className={`${page === "trash" ? "me-1" : "d-none"}`}
                  icon={<FaTrashRestoreAlt size={"22px"} />}
                  onClick={() => {
                    navigate(`/${page === "notes" ? "" : page}`);
                    restoreNote();
                  }}
              ...
              <Tooltip label='Share' placement='top' hasArrow='true'>
                <IconButton
                  ...
                  size={"lg"}
                  isRound={true}
                  icon={<BsFillPersonPlusFill size={"25px"} />}
                  onClick={() => {
                    setModal("share");
                    onOpen();
                  }}
            ...
            <Button
              className={`me-2 ${page === "shared" ? "d-none" : ""}`}
              variant={"ghost"}
              colorScheme='white'
